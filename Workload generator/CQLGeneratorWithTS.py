...
def create_insert_query(keyspace_name,table_name,columns,J) :
...
def create_update_query(keyspace_name,table_name,columns,primary_key,num_of_insert,J) :
    # UPDATE cycling.cyclist_id SET age = 28 WHERE lastname = 'WELTEN' and firstname = 'Bram' IF EXISTS;
    query=f'UPDATE {keyspace_name}.{table_name} SET {columns[3]} = {columns[3]} + '
    query+=""
...
for i in range(num_of_queries):
    if i<num_of_insert :
        temp=create_insert_query(keyspace_name,table_name,columns,i)
        dup_in.append(temp)
        insert.append(temp[:-1]+f' USING TIMESTAMP {i+1};')
    elif i<num_of_insert+num_of_update :
        temp=create_update_query(keyspace_name,table_name,columns,primary_key,num_of_insert,i)
        dup_up.append(temp)
